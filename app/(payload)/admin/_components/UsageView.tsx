 ...
 * Data from logs_ai_runs. Shown under /admin/usage so the admin nav is visible.
 */
...
export function UsageView() {
  ...
  function fetchUsage() {
    ...
    fetch(`/api/admin/ai-usage?${params}`, { credentials: 'include' })
      .then((r) => {
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-4">
          <Link href="/admin/ai-models" className="text-sm text-muted-foreground hover:text-foreground">
            ← Admin
          </Link>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <Link href="/admin/run-logs" className="text-sm text-muted-foreground hover:text-foreground">
          Run logs (pipeline)
        </Link>
      </div>
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-lg border bg-card p-4">
              ...
              <p className="mt-1 text-2xl font-semibold">${data.summary.totalCost.toFixed(4)}</p>
            </div>
            <div className="rounded-lg border bg-card p-4">
              <p className="text-sm font-medium text-muted-foreground">Total tokens</p>
              <p className="mt-1 text-2xl font-semibold">{data.summary.totalTokens.toLocaleString()}</p>
            </div>
            <div className="rounded-lg border bg-card p-4">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.byModel.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-4 text-center text-muted-foreground">No data in range</td>
                      </tr>
                    ) : (
                      data.byModel.map((row) => (
                        <tr key={row.model} className="border-b last:border-0">
                          <td className="p-2" title={row.model}>{shortModel(row.model)}</td>
                  ...
                  {data.rows.length === 0 ? (
                    ...
                    data.rows.map((row) => (
                      <tr key={row.id} className="border-b last:border-0 hover:bg-muted/20">
                        ...
                        <td className="p-2">{row.eventType}</td>
                        <td className="p-2 max-w-[120px] truncate" title={row.projectName ?? ''}>{row.projectName ?? '—'}</td>
                        <td className="p-2 max-w-[160px] truncate" title={row.userEmail ?? ''}>{row.userEmail ?? '—'}</td>
                        <td className="p-2" title={row.model ?? ''}>{shortModel(row.model)}</td>
                        <td className="p-2 text-right">{row.totalTokens != null ? row.totalTokens.toLocaleString() : '—'}</td>
                        <td className="p-2 text-right">{row.cost != null ? `$${row.cost.toFixed(4)}` : '—'}</td>
